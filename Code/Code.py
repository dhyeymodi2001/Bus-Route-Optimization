...
def bus_scheduling_optimization_with_min_trips(demand, trip_factors, fleet_size, min_trips_per_shift):
    solver = pywraplp.Solver.CreateSolver('SCIP')
    ...
    for j in range(num_shifts):
        solver.Add(sum(x[i, j] + y[i, j] for i in range(num_routes)) >= min_trips_per_shift[j])
...
print("\nModified code output (with min trips):")
bus_scheduling_optimization_with_min_trips(demand, trip_factors, fleet_size, min_trips_per_shift)